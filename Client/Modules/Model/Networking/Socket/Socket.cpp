...
Socket::Socket(std::shared_ptr<Message_From_Server_Queue> message_queue,
    ...
    {
        m_message_factory = std::make_unique<TFactory<std::string, IMessage>>(Message_Factory_Config::Message_Factory_Config());
    }
}
...
void Socket::Send_Socket() {
    if(!m_message_to_server_queue->Empty()) {
        auto message = m_message_to_server_queue->Pop_Message();
...
void Socket::Socket_Start() {
    ...
    m_io_context->post([&]{Listen_Socket();});
    m_io_context->post([&]{Send_Socket();});
    m_io_context->run();
}
