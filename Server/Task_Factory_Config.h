...
std::unique_ptr<Task_Factory> Task_Factory_Config(std::unique_ptr<Pool_Connection> poolConnection){
    auto task_factory = std::make_unique<Task_Factory>(std::move(poolConnection)) ;

    task_factory->Register_Task("authorization_task", std::make_unique<Authorization_Task_Creator>());
    task_factory->Register_Task("registration_task",std::make_unique<Registration_Task_Creator>());
