...
std::pair<std::shared_ptr<Client_Socket>, std::vector<char>> Client_Socket::Listen_Socket() {
    ...
    try {
        ...
        std::pair<std::shared_ptr<Client_Socket>, std::vector<char>> ans = std::make_pair(std::make_shared<Client_Socket>(*this), client_buffer);
        return ans;
...
Client_Socket::Client_Socket(std::shared_ptr<Client_Socket_Manager> manager, std::shared_ptr<MySocketType> socket):m_socket(socket) {
    if(manager)
